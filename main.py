...
class VSC(FastEnum):
    RANDOM = 0
    AutoCoreset_CARATHEODORY = 1
    AutoCoreset_MEDIAN_OF_MEANS = 2
    AutoCoreset_IMPORTANCE_SAMPLING = 3
...
def generateVSCoreset(Q, m, **kwargs):
    ...
    for i in range(len(kwargs['size_per_class'][0])):
        ...
        elif kwargs['VSC'] is VSC.AutoCoreset_MEDIAN_OF_MEANS:
            temp_idxs, temp_weights = medianOfMeans(Q[kwargs['size_per_class'][0][i]],
                                                    kwargs['size_per_class'][1][i])
        elif kwargs['VSC'] is VSC.AutoCoreset_CARATHEODORY:
        ...
        elif kwargs['VSC'] is VSC.AutoCoreset_IMPORTANCE_SAMPLING:
...
def initiateAutoCore(P, y, f, m, eta, zeta, model_f, **kwargs):
    ...
    for i in range(eta):
        if kwargs['randomized_sol'] == 'uniform':
            C, y_C, w_C = generateUniformCoreset(P, y, m, w=kwargs['old_weights'], replace=kwargs['replace'],
                                                 maintain_classes=kwargs['maintain_classes'])
            sol, _ = coreset_utils.fitModel(C, y_C, w_C, model_f, np.sum(kwargs['old_weights']))
    ...
    while zeta(M_f):
        if np.any(M_f.shape[1] >= kwargs['size_per_class'][1]) and kwargs['VSC'] is VSC.AutoCoreset_CARATHEODORY:
        ...
        sol, _ = coreset_utils.fitModel(C, y_C, w_C, model_f, sum_old_weights=np.sum(kwargs['old_weights']))
...
def printResults(vals, acc, times, labels,problem_type):
    print(f'Method \t eps \t {"acc" if problem_type == "classification" else "R^2"} \t time \t')
    for i in range(len(labels)):
        if labels[i] == 'RANDOM': labels[i]+="                 "
        print('{} \t {} \t {} \t {} \t'.format(labels[i], vals[i], acc[i], times[i]))


def main():
    global INITIAL_RANDOM_SOLS
    file_names = ['Data/HTRU_2.csv']
    for file_name in file_names:
        print('************************** {} ***************************'.format(file_name))
    
        problems_choice = [ 'k_means', 'logistic_regression',  'linear_regression', 'svm',]
        for problem in problems_choice:
            file_name_save = 'Results/' + file_name.split('/')[-1].split('.')[0] + '_' + problem 
            ...
            X_train, y_train, X_test, y_test, is_multiclass = coreset_utils.readDataset(file_path=file_name, standarize=True,
            ...
            loss, model_f = coreset_utils.obtainLossAndModel(problem, **model_parameters)
            ...
            start_time_all = time()
            opt_sol, opt_model = coreset_utils.fitModel(X_train, y_train, sample_weight=None, model_f=model_f,
            ...
            if 'means' in problem:
                ...
                print('Optimal score is ', opt_acc)
                if problem_type == 'classification':
                    y_pred_all = opt_model.predict(X_test)
                    #confusion_mat = coreset_utils.confusion_matrix(y_test, y_pred_all)
            ...
            for coreset_size_idx in range(coreset_sizes.shape[0]):
                ...
                if problem_type == 'classification':
                    chunks, size_per_class = \
                        coreset_utils.chunkize(np.arange(X_train.shape[0]),
                    ...
                    _, _, chunk_idxs, size_per_chunk =\
                        coreset_utils.shuffleDataViaChunks(X_train,
                                                   y_train if 'multiclass' not in model_f[1] else y_train.argmax(axis=1),
                                                   chunks)
                else:
                ...
                for i in tqdm(range(competing_methods)):
                    ...
                    for trial in range(trials):
                        ...
                        temp_acc[trial], temp_vals[trial] = coreset_utils.evaluateCoreset(C, y_c, v, X_test, y_test, model_f,
                ...
                printResults(mean_vals[:, coreset_size_idx], mean_acc[:, coreset_size_idx],
                             mean_times[:, coreset_size_idx], method_labels, problem_type)
